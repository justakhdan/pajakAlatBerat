...
<title>DATA KARYAWAN - UBAH ::: PAJAK ALAT BERAT - KOTA DUMAI</title>
...
if(!isset($namaSes)){
	header("Location:./index.php");
		...
		$dataUser = mysqli_query($conn, "select datakaryawan.*, login.* from datakaryawan, login where datakaryawan.kodeAkses=login.kodeAksesKaryawan and datakaryawan.kodeAkses='$kodeAkses'");
 if (!$dataUser) {
    printf("Error: %s\n", mysqli_error($conn));
    exit();
}
...
if(isset($_POST['edit'])){
	...
	$update = mysqli_query($conn, "UPDATE datakaryawan SET nama='$nama', alamat='$alamat', nomorTelepon='$nomorTelepon', email='$email' WHERE kodeAkses='$kodeAkses'");
	$update2 = mysqli_query($conn, "update login set username='$username', password='$password' where kodeAksesKaryawan='$kodeAkses'");
...
<header>
  ...
  <br /><br />
  <form name="editKaryawan" method="POST">
