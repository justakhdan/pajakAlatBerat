...
<title>DATA ALAT BERAT PERUSAHAAN - DETAIL ::: PAJAK ALAT BERAT - KOTA DUMAI</title>
...
while($res = mysqli_fetch_array($dataInvent))
{
	...
	$tanggalJatuhTempo= strtoupper(date('d F Y', strtotime($res['tanggalJatuhTempo'])));
	
	}
	$tanggalSekarang = strtoupper(date('d F Y'));
	...
	$queryDenda = mysqli_query($conn, "select * from dendapajak where indexDenda='$indexDenda'");
...
if(isset($_POST['bayar'])){
	$tanggalBaru = date('Y/m/d', strtotime($tanggalJatuhTempo. ' +365 day'));
	$tanggalUpdate = date('Y/m/d');
	$tanggalJT = date('Y/m/d', strtotime($tanggalJatuhTempo));
	if(strtotime($tanggalJatuhTempo) > strtotime($tanggalSekarang)){
	$updateBiaya = $nilaiPajak;
	$sql = mysqli_query($conn, "update inventaris set tanggalJatuhTempo='$tanggalBaru' where kodeInvent='$kodeInvent'");
	$sql1= mysqli_query($conn, "insert into riwayatpajak (kodeInvent, tanggalPembayaran, tanggalJatuhTempo, nilaiPajak, denda, jumlah, keteranganBayar) values ('$kodeInvent', '$tanggalUpdate','$tanggalJT', '$nilaiPajak', '0', '$updateBiaya', '$keterangan')");
		}
	else{
		$updateBiaya = $denda;
	$sql = mysqli_query($conn, "update inventaris set tanggalJatuhTempo='$tanggalBaru' where kodeInvent='$kodeInvent'");
	$sql1= mysqli_query($conn, "insert into riwayatpajak (kodeInvent, tanggalPembayaran, tanggalJatuhTempo, nilaiPajak, denda, jumlah, keteranganBayar) values ('$kodeInvent', '$tanggalUpdate', '$tanggalJT', '$nilaiPajak', '$updateDenda', '$updateBiaya', '$keterangan')");
		}	
	
	header("Location: dabusdetail.php?kodeInvent=$kodeInvent");	
...
<?php } ?>
  ...
  <form name="updateNJKB" method="POST">
  <table>
  <tr><td colspan=6><br /><br /></td></tr>
  <tr>
  ...
  <br />
  <input type="submit" name="bayar" id="bayar" value=" Bayar " <?php if ($selisih > '60'){?> disabled="disabled" <?php };?> />
